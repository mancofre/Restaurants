            ...
            }
            {
                size(reviews) > 0 && (
                    map(reviews, reviewDocument => (
                        <Review key={reviewDocument.id} reviewDocument={reviewDocument}/>
                    ))
                )
            }
        </View>
...
function Review({ reviewDocument }) {    
    const {id, title, review, createAt, avatarUser, rating } = reviewDocument 
    const createReview = new Date(createAt.seconds * 1000)
    
    return (
        <View style={styles.viewReview} key={id}>
